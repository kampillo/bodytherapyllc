// src/components/admin/BlogList.tsx - Versión mejorada con mejor UI
'use client';
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Posts del Blog</h2>
          <p className="text-gray-600 mt-1">Gestiona todos los artículos de tu blog ({posts.length} posts)</p>
        </div>
      ...
      {posts.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <p className="text-gray-500 mb-6">Comienza creando tu primer post del blog.</p>
          <Button href="/admin/blog/new" variant="primary">
            <svg className="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
            </svg>
        ...
        <>
          ...
          {viewMode === 'grid' && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {posts.map((post) => (
                <div key={post.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-200 group">
                  {/* Imagen */}
